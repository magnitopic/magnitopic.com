...
export default function Home() {
	return (
		<main className="min-h-screen overflow-x-hidden">
			...
			<div
				aria-hidden="true"
				className="fixed top-1/4 left-1/4 w-[500px] h-[500px] bg-blue-700/8 rounded-full blur-3xl pointer-events-none -z-10"
			/>
			<div
				aria-hidden="true"
				className="fixed bottom-1/3 right-1/4 w-72 h-72 bg-blue-500/5 rounded-full blur-3xl pointer-events-none -z-10"
			/>
			...
			>
				<div className="relative z-10">
					<p
						className="text-xs text-blue-400/70 font-mono tracking-[0.25em] uppercase mb-6"
						aria-hidden="true"
					>
						Hi, I&apos;m
					</p>
					...
					<h1
						id="hero-name"
						className="text-7xl md:text-9xl font-bold leading-[0.9] mb-6 pb-2"
					>
					...
					<div className="flex items-center gap-3 mb-5">
						...
						<span className="text-blue-300/80 font-mono text-sm">
							Magnitopic
						</span>
						<span
							aria-hidden="true"
					...
					<p className="text-lg text-white/70 max-w-lg mb-8 leading-relaxed">
						Building data pipelines and making sense of large-scale
						data. Currently a Junior Data Engineer at Quantori,
						working in the medical sector. Also studying a
						Master&apos;s in AI &amp; Big Data and specializing
						through 42 Network&apos;s Data &amp; AI track.
					</p>

					<ul
						aria-label="Areas of interest"
						className="flex flex-wrap gap-2 mb-10 list-none"
					>
						{[
							"Data Engineering",
							"AI / ML",
							...
							<li
								key={tag}
								className="text-xs px-3 py-1 rounded-full border border-blue-500/30 text-blue-300/80 font-mono"
							>
								{tag}
							</li>
						))}
				...
				<div
					aria-hidden="true"
					className="absolute bottom-10 left-6 flex flex-col items-center gap-2 text-white/20"
				>
					<div className="w-px h-16 bg-gradient-to-b from-transparent via-white/20 to-transparent" />
					<span className="text-[10px] font-mono tracking-[0.2em] rotate-90 origin-center translate-y-6">
						scroll
					</span>
				</div>
			</section>
			...
			>
				...
				{/* Timeline */}
				<ol
					className="relative mt-12 list-none"
					aria-label="Career timeline"
				>
					{/* Vertical line running through the whole list */}
					<div
						aria-hidden="true"
						className="absolute left-[3.25rem] top-3 bottom-3 w-px bg-white/[0.07]"
					/>
					...
					{experience.map((job, i) => (
						...
						>
							...
							<div className="flex flex-col items-center shrink-0 w-16">
								<div
									className={`relative z-10 text-xs font-mono px-1.5 py-1 rounded-md ${job.current ? "bg-blue-500/15 text-blue-300 border border-blue-500/30" : "bg-white/[0.04] text-white/40 border border-white/[0.08]"}`}
								>
									{job.year}
								</div>
							</div>
							...
							{/* Card */}
							<div
								className={`flex-1 rounded-xl border p-5 mb-2 transition-colors ${job.current ? "border-blue-500/25 bg-blue-950/10" : "border-white/[0.07] bg-white/[0.02]"}`}
							>
								<div className="flex flex-wrap items-center gap-2 mb-1">
									<h3
										className={`text-base font-medium ${job.current ? "text-white" : "text-white/85"}`}
									>
										{job.role}
									</h3>
									{job.current && (
								...
								<ul
									className="flex flex-wrap gap-1.5 list-none"
									aria-label={`Technologies used at ${job.company}`}
								>
									{job.tags.map((tag) => (
										<li
											key={tag}
											className="text-xs px-2.5 py-1 rounded-md border border-white/[0.08] bg-white/[0.03] text-white/55 font-mono"
										>
											{tag}
										</li>
									))}
						...
						>
							...
							>
								{project.tags.slice(0, 4).map((tag) => (
									<span
										key={tag}
										className="text-[10px] text-blue-400/60 font-mono"
									>
										#{tag}
									</span>
								))}
							...
							<div className="mt-5 pt-4 border-t border-white/[0.06] flex items-center justify-between">
								...
								>
									<i
										className="fa fa-github text-sm"
										aria-hidden="true"
									/>
									GitHub
								</a>
...
function SectionLabel({ number, label, id }) {
	return (
		<div className="flex items-center gap-3">
			<span
				aria-hidden="true"
				className="text-blue-500/50 font-mono text-xs"
			>
				{number}
			</span>
			...
			<h2
				id={id}
				className="text-xs font-mono uppercase tracking-[0.2em] text-white/55"
			>
				{label}
			</h2>
		</div>
